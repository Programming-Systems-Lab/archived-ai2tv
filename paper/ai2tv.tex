...
the online curriculum has draw attention to the disparity in the
network resources used by the students.  We present an architecture
and adaptation model called $\mathrm{AI}^2$TV, a system that allows participants
that are disadvantaged in network resources and geographically
dispersed to collaboratively view a video in synchrony.  We uphold the
invariant that each participant will view semantically equivalent
content at all times.  Video player actions, like play, pause and
stop, can be initiated by any of the participants and the results of
those actions are seen by all the members.  These features allow group
members to review lectures videos in tandem to facilitate the learning
process.  We employ an autonomic (feedback) controller that monitors
client video status and adjusts the quality of the video according to
the resources of each client.  We show that our system can
successfully synchronize video for distributed clients while at the
same time optimizing the video quality by adaptively adjusting the
quality level for each participant.
\end{abstract}
...
solution within the $\mathrm{AI}^2$TV project.  We then present the criteria used
...
% - ai2tv project goals
The work presented in this paper is a part of the $\mathrm{AI}^2$TV project, a
multi-group effort aimed at developing a collaborative virtual
environment for distributed team study.  $\mathrm{AI}^2$TV differs from many
...
$\mathrm{AI}^2$TV, an important goal is to provide support for the collaborative
...
One of the requirements of the multimedia support within the $\mathrm{AI}^2$TV
...
$\mathrm{AI}^2$TV, we take advantage of a semantic summarization package - also
...
Through the use of the $\mathrm{AI}^2$TV video, we can provide semantically
...
controlled $\mathrm{AI}^2$TV video system and employs a software based workflow
...
the context of $\mathrm{AI}^2$TV, Workflakes coordinates the adjustment of the
compression level assigned to each client along the hierarchy of the
$\mathrm{AI}^2$TV video.
...
for viewing.  The provided content has the form of an $\mathrm{AI}^2$TV video,
...
clients, as well as between the $\mathrm{AI}^2$TV system and the autonomic
...
player actions pertain to the functionality of the $\mathrm{AI}^2$TV system, since
...
\textit{Evaluating Quality of Service} \\ A primary goal of the $\mathrm{AI}^2$TV
...
The primary goals of $\mathrm{AI}^2$TV are distributed client video synchrony
...
(i.e., the $\mathrm{AI}^2$TV video buffer and manager).  Their approach employs
