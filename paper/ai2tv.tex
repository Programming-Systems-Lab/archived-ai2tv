...
that each participant will view semantically equivalent content at all
...
The video display renders the JPG frames into a window for display
...
Our experiments for the evaluation of synchronization iniitially
...
provider, but she may actually be receiving a significanty lower rate.
...
thus missd an average of 63\% of the needed frames.  In the latter
...
weighted score is calculated as the ratio of the fram rate of the two
...
\texttt{n} fps while the level higher plays at \texttt{2*n} fpm, the
...
whether they take action reactiely or pro-actively, and whether it
...
adpative scheme that employs a global clock and operates in a
...
peer-to-peer architecutre to implement a synchronization scheme, and
